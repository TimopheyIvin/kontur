...
public:
	...
	Value getValue(char ch, int number)
	{
		...
		{
			for (int i = number; i < M; i++) // если ключ не найден в value, ищет его среди составных высказываний и вычисл€ет
			{
				if (operation[i][0] == ch)
				{
	...
	{
		Value left, right;
		if (!(str.find('|') != str.npos) & !(str.find('&') != str.npos)) // дл€ обработки случаев, подобных этому: 1 1 A 0 B = A
		{
	...
	void count()
	{
		for (int i = 0; i < M; i++)
		{
			...
			for (int j = i + 1; j < M; j++)// своеобразный костыль, дл€ корректной обработки следующих строк и снижени€ сложности алгоритма с ќ(2^N) до O(N^3)
			{
...
void main(int argc, char* argv[])
{
	...
	{
		cout << "Ќеобходимо указать два аргумента!";
	}
}
